...
import LockInTeam from "../../../components/LockInTeam";
...
export default function MatchChatPage() {
  ...
  // Determine if this user has locked in their team (the FIX is here)
  useEffect(() => {
    if (!match || !user) return setLockedIn(false);
    const isCreator = user.uid === match.creatorUserId;
    // Check for team + gamertag for creator or joiner
  ...
  if (!user)
    return <div className="text-red-400 p-6">Please log in to chat.</div>;
  if (!match)
    return <div className="text-white p-6">Loading match info...</div>;
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-gradient-to-br from-[#191926] via-[#29184b] to-[#12001f] py-10 px-2 relative overflow-hidden">
      ...
      <div className="w-full max-w-2xl rounded-3xl bg-[#201a37] shadow-2xl px-6 py-8 flex flex-col items-center relative z-10">
        <h1 className="text-3xl sm:text-4xl font-extrabold mb-5 text-yellow-300 tracking-tight drop-shadow-[0_0_8px_rgba(255,230,100,0.8)]"
        ...
        <div className="w-full flex flex-col gap-5">
          ...
          <LockInTeam matchId={matchId as string} match={match} onLocked={() => {}} />
          <ConfirmInGameButton matchId={matchId as string} match={match} disabled={!lockedIn} />
        </div>
      </div>
    </div>
  );
