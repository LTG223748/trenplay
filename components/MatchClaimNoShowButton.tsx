// components/MatchClaimNoShowButton.tsx
...
type Props = {
  matchId: string;
  scheduledAt: FireTime;                    // match.scheduledAt (ms | Date | string | Firestore Timestamp)
  claimantUid: string;                      // current user's uid
  participantUids?: (string | undefined)[]; // [creatorUid, opponentUid]
  onSuccess?: (winnerUid: string) => void;  // optimistic callback
};
...
export default function MatchClaimNoShowButton({
  matchId,
  scheduledAt,
  claimantUid,
  participantUids = [],
  ...
  const ready = msRemaining === 0 && isParticipant && !!matchId;
  ...
  return (
    <button
      ...
      title={
        !isParticipant
          ? "Only players in this match can claim"
          : msRemaining > 0
